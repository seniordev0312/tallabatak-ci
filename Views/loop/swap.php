 ...
 ?>
  ...
  <?php if ($posts): ?>
    <?php foreach ($posts as $key => $value): ?>
     ...
     $check = false; 
      if($this->user_id){
            $check = $this->common_model->GetSingleData('wishlist' , array('post_id' => $value['id'],'post_type' => 'swap' ,'user_id'=>$this->user_id)); 
      }
      ...
      if (!$user_info) {
       ...
       if ($this->user_id)
       {
           ...
           $link_attr = 'href="'.base_url().'/swap-detail/'.$value['id'].'"';
        }
        else
        {
            ...
            <div class="card shadow-sm card-hover border-0 h-100">
                <div class="tns-carousel-wrapper card-img-top card-img-hover"><a class="img-overlay" <?= $link_attr ?>></a>
                    ...
                    <div class="content-overlay end-0 top-0 pt-3 pe-3">
                      <button class="btn btn-icon btn-light btn-xs text-primary rounded-circle " type="button" data-bs-toggle="tooltip" data-bs-placement="left" title="Add to Wishlist" onclick="return addToFav(<?php echo $value['id'] ?> , 'swap')"><i class="fa <?= ($check) ? 'fa-heart' : 'fa-heart-o' ?> post_heart_<?= $value['id'] ?>"></i></button>
                    </div>
                    <div>
                ...
                <div class="card-body position-relative pb-3">                                
                    <h3 class="h6 mb-2 mt-1 fs-base">
                        <a class="nav-link stretched-link" <?= $link_attr ?>><?= $value['title']?> </a>
                    </h3>
