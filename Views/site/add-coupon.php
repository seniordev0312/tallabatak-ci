...
<div class="container">
    ...
    <!-- my code -->
              <div class="table-responsive table-card mt-3 mb-1">
                 <h1 class="h2 mb-0">Add Coupon</h1>
        ...
        <div class="col-md-12 py-3">
            ...
            <div>
                ...
                <input type="date" min="<?= date('Y-m-d'); ?>" class="form-control"  name="end_date" id="end_date" required>
            </div>
            ...
            <div>
                ...
                <label><input type="radio" name="invite" class="invite_radio" value="invite_city"> Invite By City</label><br>
                  <div class="city_select other_select" style="display: none; ">
                    <label>Users of "<?= $this->user['city'] ?>"</label>
                    <select class="form-control user_page" id="city_near_user"  name="near_user[]" multiple="">
                      <?php
                      
                      $userData_near = $this->common_model->GetAllData('users',array('city'=>$this->user['city'],'id!='=> $id),'id','desc');
                        if($userData_near){
                ...
                <div class="country_select other_select" style="display: none; ">
                  <label>Users of "<?= $this->user['country'] ?>"</label>
                  <select class="form-control user_page" id="country_near_user"  name="near_user[]" multiple="">
                     <?php
                     $userData_near = $this->common_model->GetAllData('users',array('country'=> $this->user['country'],'id!='=> $id),'id','desc');
                      if($userData_near){
                      foreach($userData_near as $UserNear){
                    ?>
                    <option value="<?= $UserNear['id']; ?>"><?= $UserNear['name']; ?></option>
